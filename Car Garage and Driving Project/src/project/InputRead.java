...
public class InputRead {
	...
	public static void setDelimeter() {
		entry.useDelimiter("[,\n\r]+");
	}
	...
	static boolean createRandom() {
		while(true) {
			char op = option();
			if(op!='c'&&op!='r')System.out.println("Not valid");
			else if(op=='c')return true;
			else return false;
		}
	}
	...
	static public char option() {		
		String s="";
		while(s.length()==0)s = entry.nextLine();
		return s.charAt(0);
	}
