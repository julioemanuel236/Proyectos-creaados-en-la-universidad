...
public final class GestorMovimiento {
	...
	private static boolean agregando = false;
	...
	public static void actualizar() {
		...
		int n = lista.size();
		//System.out.println("CANTIDAD MOVIMIENTOS A ACTUALIZAR:" + n);
		long time = System.currentTimeMillis();
		for(int i=0;i<n;i++) {			
			//if(lista.size()>n)System.out.println("HUBO CAMBIO EN EL TAMAÑO DE LA LISTA");
			...
			if(p!=null) {
				...
				if(!p.yaTermino()) {
					while(agregando) {
						//System.out.println("ESPERANDO PARA AGREGAR");
						try {
							Thread.sleep(1);
						}
						catch(Exception e) {
					...
					//System.out.println("LIBERANDO BOLITA");
					((Bolita)p.getComponent()).setMoviendo(false); 
				}
			}
	...
	public static void run() {		
		//System.out.println("EJECUCIO INICIADA");
