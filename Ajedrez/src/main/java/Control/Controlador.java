...
public class Controlador {
        ...
        if (rey != null) {
            for (Pieza pieza : getPiezas()) {
                if (pieza.getColor() != colorRey) {
                    ArrayList<int[]> movimientos = obtenerMovimientosValidos(pieza);
                    for (int[] movimiento : movimientos) {
                        if (movimiento[0] == rey.getX() && movimiento[1] == rey.getY()) {
    ...
    public boolean puedenBloquear(TColor color) {
        Pieza rey = obtenerRey(color);
        TColor colorOpuesto;
        ...
        if (rey != null) {
            ArrayList<Pieza> piezasAtacantes = obtenerPiezasAtacantes(colorOpuesto);

            for (Pieza atacante : piezasAtacantes) {
                ArrayList<int[]> movimientosAtacante = obtenerMovimientosValidos(atacante);
                for (int[] movimiento : movimientosAtacante) {
                    int xAtacante = movimiento[0];
                    int yAtacante = movimiento[1];

                    for (Pieza pieza : Piezas) {
                        if (pieza.getColor() == color) {
                            ArrayList<int[]> movimientosPieza = obtenerMovimientosValidos(pieza);
                            for (int[] movimientoPieza : movimientosPieza) {
                                int xPieza = movimientoPieza[0];
                                int yPieza = movimientoPieza[1];

                                // Si el movimiento de la pieza bloquea el ataque
                                if (xPieza == xAtacante && yPieza == yAtacante &&
                                    realizarMovimiento(pieza, xAtacante, yAtacante)) {
                                    // Si el rey ya no está en jaque, se puede bloquear el ataque
                                    if (!jaque(color)) {
                                        deshacerMovimiento(pieza, xAtacante, yAtacante, null);
                                        return true;
                                    }
                                    deshacerMovimiento(pieza, xAtacante, yAtacante, null);
                                }
        ...
        return false; // No pueden bloquear el ataque al rey
    }
