...
public abstract class Enemigo extends Mob {
	int vision;	
	public Enemigo(int vida, int damage,int mM,boolean volar,char c,int puntaje) {
	...
	public boolean mover() {
		if(inRange()&&App.player.tablero.dificultad==3) {
			perseguir();
			return true;
		}
	...
	public void perseguir() {
		if(!inRange())return;
		int rx=x,ry=y;
		int dmin=App.player.tablero.width*App.player.tablero.height;
		int arr[]= {0,2,4,6};
		for(int i=0;i<4;i++) {
			int dir=arr[i];
	...
	public int distanciaManhatanToHero(int x,int y) {
		Heroe e = App.player.ingame[App.player.mainHero];
